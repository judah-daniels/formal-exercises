...
`ifdef	FORMAL
  ...
  always @(posedge glb_clk) begin
    // i_fast_clk is exactly WIDTH times faster than i_slow_clk
    f_clk_counter <= f_clk_counter + 1'b1;       

    assume(i_fast_clk == f_clk_counter[0]);
    assume(i_slow_clk == f_clk_counter[WIDTH-1]);

    // i_slow_clk and i_fast_clk share a positive edge transition
    if (f_past_valid && $rose(i_slow_clk)) 
      assume($rose(i_fast_clk));
  ...
  // Proofs
  always @(posedge glb_clk) begin
    // The output return on the positive edge of the slow clock , is the input
    // that was received
  end

  always @(posedge glb_clk) begin
    // The slow clock outputs are stable, over the slow clock
  end
