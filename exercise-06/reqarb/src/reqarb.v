...
`ifdef	FORMAL
    // Prove that no data will be lost
    //

    // TEMP - assume i_busy is always false
    always @(*)
        assume(!i_busy);

    always @(posedge i_clk) begin
        if (i_a_req && o_a_busy || i_busy) begin
            assume($stable(i_a_req && i_a_data));
        end
        if (i_b_req && o_b_busy || i_busy) begin
            assume($stable(i_b_req && i_b_data));
        end
        if (o_req && i_busy) begin
            assert($stable(o_req && o_data));
        end
    end
